...
. {{ m.pkgdatadir }}/atomic/include.sh
...
chroot "${HOST}" /usr/bin/docker create -v "${data_dir}:{{ m.pgdata }}:Z" \
    --name "${NAME}" ${OPT2} "${IMAGE}" ${OPT3}

# Create and enable systemd unit file for the service
sed -e "s/TEMPLATE/${NAME}/g" "{{ m.pkgdatadir }}/atomic/"template.service > "${HOST}/etc/systemd/system/${service_name}.service"
